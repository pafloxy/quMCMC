...
## mcmc types 
mcmc_types = ['qu-wt1', 'qu-wt2' ]

name = "TrainingExperiments/BAS3/TRAININGEXPERIMENT_1_BAS3.pkl"

## to reinitiate experiment
# TRAININGEXPERIMENT_1 = {mcmc_type: cd_training(param_model, beta_train, DATA_b3, name= mcmc_type) for mcmc_type in ['qu-wt1', 'qu-wt2' ] }
...
for mcmc_type in mcmc_types :
    TRAININGEXPERIMENT_1[mcmc_type].train(lr =lr, mcmc_settings= MCMC_SETTINGS[mcmc_type], epochs= EPOCHS, mcmc_steps= MCMC_STEPS, verbose= True, update_strategy= ['random', {'num_random_bias': 9, 'num_random_interactions': 27}])
