...
def fn_qckt_X_mixer(
    ...
    if mixer_type[0] == "custom":
        ...
        possible_qubit_combinations = [
            list(i) for i in combinations(qubit_indices, mixer_type[1])
        ]  # in ascending order
...
def run_qmcmc_quantum_ckt(
    ...
    qc_problem_half = fn_qckt_problem_half(
        J=J, h=h, num_spins=num_spins, gamma=gamma, alpha=alpha, delta_time=delta_time
    )
    qc_mixer = fn_qckt_X_mixer(
        num_spins=num_spins, gamma=gamma, delta_time=delta_time, mixer_type=mixer_type
    )
