...
class IsingEnergyFunction:
    ...
    def model_summary(self, plot= True) :
        ...
        print("Non-zero Interactions (J) : "+ str( np.count_nonzero(self.J) /2) + ' / '+str(  0.5 * self.num_spins * (self.num_spins - 1)) )
        print("Non-zero Bias (h) : "+ str( np.count_nonzero(self.h) / 2) + ' / ' + str( self.num_spins ) )
        print("---------------------------------------------")
