...
def fn_qc_h1(num_spins: int, gamma, alpha, h: list, delta_time=0.8) -> QuantumCircuit:
    ...
    for j in range(0, num_spins):
        unitary_gate = DenseMatrix(
            index=num_spins - 1 - j,
            matrix=np.round(
                expm(
                    -1j
                    * delta_time
                    * (gamma * X(2).get_matrix() + b_list[j] * Z(2).get_matrix())
                ),
                decimals=6,
            ),
        )
...
def fn_qc_h2(J: np.array, alpha: float, gamma: float, delta_time) -> QuantumCircuit:
    ...
    theta_array = (
        2 * -1 * (1 - gamma) * alpha * delta_time
    ) * J  # *2 is intentional because rotation operator has a factor of 2 in Denominator in its phase.
...
def trottered_qc_for_transition(
    num_spins: int, qc_h1: QuantumCircuit, qc_h2: QuantumCircuit, num_trotter_steps: int
) -> QuantumCircuit:
...
def run_qc_quantum_step(
    ...
    gamma = np.round(
        np.random.uniform(low=min(gamma_range), high=max(gamma_range)), decimals=6
    )
    ...
    qc_time_evol = trottered_qc_for_transition(
